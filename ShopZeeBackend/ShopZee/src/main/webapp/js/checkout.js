...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
	...
	const res = await fetch(`/ShopZee/cart/${userId}`);
    ...
    for (const item of rawCartItems) {
	  ...
	  const productRes = await fetch(`/ShopZee/api/products/${item.productId}`);
  ...
  if (confirmBtn) {
    confirmBtn.addEventListener("click", async function () {
      try {
		...
		const cartItemsRes = await fetch(`/ShopZee/cart/${userId}`);
        ...
        //const orderRes = await fetch(`http://localhost:8080/orders/place`, {
		  const orderRes = await fetch(`/ShopZee/orders/place`,{
          method: 'POST',
        ...
        for (const item of latestCart) {
		  ...
		  await fetch(`/ShopZee/cart/remove/${item.cartItemId}`, { method: 'DELETE' });
        }
